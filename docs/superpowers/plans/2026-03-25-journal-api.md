...
**Goal:** Add 6 read-only journal API endpoints to the existing api-gateway worker for serving trade journal data to the web dashboard.

**Architecture:** New route file `routes/journal.ts` mounted on the existing protected app (JWT auth + rate limiting applied automatically). All queries against the existing `journal_trades` D1 table. Follows the signals.ts pattern for dynamic WHERE clause building and cursor pagination.

**Tech Stack:** Hono, TypeScript, D1/SQLite, existing JWT auth middleware

**Spec:** `docs/superpowers/specs/2026-03-25-journal-api-design.md`
...
|------|--------|----------------|
| `workers/api-gateway/src/routes/journal.ts` | Create | All 6 journal API route handlers |
| `workers/api-gateway/src/index.ts` | Modify | Mount journal routes on protected app |

---

## Task 1: Create Journal Route File

**Files:**
- Create: `workers/api-gateway/src/routes/journal.ts`

- [ ] **Step 1: Create the complete journal routes file**
...
```bash
git add workers/api-gateway/src/routes/journal.ts
git commit -m "feat(journal): add 6 journal API route handlers"
```

---

## Task 2: Mount Journal Routes
...
Add the import at line 12 (after the news import):

```typescript
import { journal } from './routes/journal.js';
```

Add the route mount at line 76 (after the news mount):
...
git commit -m "feat(journal): mount journal routes on protected app"
```
...
Run: `curl https://edgerelay-api.ghwmelite.workers.dev/health`
